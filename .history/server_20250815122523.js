...
// Charger les routes publiques (sans middleware d'authentification)
const publicRoutes = [
  { path: './routes/users.js', route: '/api/users' },
  { path: './routes/auth.js', route: '/api/auth' }
];

publicRoutes.forEach(({ path: routePath, route }) => {
  const fullPath = path.join(__dirname, routePath);
  console.log(`🔍 Checking public route file: ${fullPath}`);

  if (fs.existsSync(fullPath)) {
    try {
      console.log(`📁 Public route file exists, attempting to load: ${routePath}`);
      const routeModule = require(routePath);
      app.use(route, routeModule);
      console.log(`✅ Successfully loaded public route: ${route}`);
    } catch (error) {
      console.error(`❌ Error loading public route ${route}:`, error.message);
      console.error(`   Full error:`, error);
    }
  } else {
    console.error(`❌ Public route file not found: ${fullPath}`);
  }
});

// Charger les routes protégées (avec middleware d'authentification)
const protectedRoutes = [
  { path: './routes/contacts.js', route: '/api/contacts' }
];

protectedRoutes.forEach(({ path: routePath, route }) => {
  const fullPath = path.join(__dirname, routePath);
  console.log(`🔍 Checking protected route file: ${fullPath}`);

  if (fs.existsSync(fullPath)) {
    try {
      console.log(`📁 Protected route file exists, attempting to load: ${routePath}`);
      const routeModule = require(routePath);
      const auth = require('./middleware/auth');
      app.use(route, auth, routeModule);
      console.log(`✅ Successfully loaded protected route: ${route}`);
    } catch (error) {
      console.error(`❌ Error loading protected route ${route}:`, error.message);
      console.error(`   Full error:`, error);
    }
  } else {
    console.error(`❌ Protected route file not found: ${fullPath}`);
  }
});
