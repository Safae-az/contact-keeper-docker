...
if curl -s -f http://localhost:5003/api/users > /dev/null; then
    echo "✅ Backend est opérationnel (endpoint /api/users  accessible)"
else
    echo "⚠️  Endpoint /api/users non accessible, test alternatif..."
